...
int main(int argc, char** argv)
{
	if(argc != 7)
	{
		std::cout << std::endl << "vis_timing_test number_of_runs number_of_terms cutoff_frequency block_length number_of_samples factor" << std::endl;
	...
	vis_config_change_number_of_samples(vis_config, true, number_of_samples);
	...
	for (int i = 0; i != number_of_terms; ++i) {
		h_constraints[i].weight = 1.0;
		h_constraints[i].position.x = h_constraints[i].position.y = h_constraints[i].position.z = 0.0;
